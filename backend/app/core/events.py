...
async def startup(app: FastAPI) -> None:
    """Initialise single-process realtime infrastructure."""
    settings = get_settings()
    app.state.websocket_manager = ConnectionManager(settings=settings)
    await app.state.websocket_manager.start()
    logger.info("runtime_started", extra={"env": settings.APP_ENV})


async def shutdown(app: FastAPI) -> None:
    """Close background listeners and active sockets cleanly."""
